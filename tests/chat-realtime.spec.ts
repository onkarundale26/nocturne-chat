...
test.describe('Nocturne Real-time Chat', () => {
  ...
  test('User A can send a message and User B receives it privately', async ({ browser }) => {
    ...
    await pageA.fill('input[placeholder="vibe_architect"]', 'UserA');
    ...
    await pageB.fill('input[placeholder="vibe_architect"]', 'UserB');
    ...
    await pageC.fill('input[placeholder="vibe_architect"]', 'UserC');
    await pageC.click('button:has-text("Join Chat")');
    await expect(pageC).toHaveURL(/.*chat/);

    // 4. User A selects User B and sends a message
    // Wait for UserB to appear and select them (using first() to avoid ambiguity)
    const userBButton = pageA.getByRole('button', { name: /UserB/ }).first();
    await expect(userBButton).toBeVisible({ timeout: 15000 });
    await userBButton.click();
    
    const messageText = `Secret vibe from User A ${Math.random()}`;
    ...
    const userAButton = pageB.getByRole('button', { name: /UserA/ }).first();
    ...
    await expect(incomingBubble).toBeVisible({ timeout: 10000 });

    // 7. Verify User C does NOT receive the message (Privacy check)
    const userAButtonForC = pageC.getByRole('button', { name: /UserA/ }).first();
